...
// a per-component conversion of the aiMatrix4x4 parameter, and is transformed into a GLM matrix:
...
{
    // Traversal starts from some node with a given parent that's passed as a parameter. A
    // new node identifier is returned by the addNode() routine shown here. If aiNode
    // contains a name, we store it in the Scene::names_ array
    ...
    // If this aiNode object has meshes attached to it, we must create one subnode for
    // each of the meshes. For easier debugging, we will add a name for each new mesh
    // subnode:
    for (size_t i = 0; i < N->mNumMeshes; i++)
    {
        ...
        // Each of the meshes is assigned to the newly created subnode. Assimp ensures that a
        // mesh has a material assigned to it, so we will assign that material to our node
        ...
        // Since we only use subnodes for to attach meshes, we will set the local and global
        // transformations to identity matrices:
        scene.globalTransform_[newSubNode] = glm::mat4(1.0f);
        scene.localTransform_[newSubNode] = glm::mat4(1.0f);
    }

    // The global transformation is set to identity at the beginning of node conversion. It
    // will be recalculated at the first frame or if the node is marked as changed.
    ...
    // At the end, we recursively traverse the children of this aiNode object:
