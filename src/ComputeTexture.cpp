...
int main()
{
    ...
    imgGen = std::make_unique<ComputedImage>(vkDev, "data/shaders/VK_compute_texture.comp", 1024, 1024, false);
    ...
    do
    {
        ...
        imgGen->fillComputeCommandBuffer(&thisTime, sizeof(float),
                                         imgGen->computedWidth / 16, imgGen->computedHeight / 16, 1);
        imgGen->submit();
