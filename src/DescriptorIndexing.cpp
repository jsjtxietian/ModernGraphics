...
void updateAnimations()
{
    ...
    {
        animations[i].textureIndex = animations[i].flipbookOffset +
                                     (uint32_t)(kAnimationFPS * ((glfwGetTime() - animations[i].startTime)));
...
bool initVulkan()
{
    ...
    // enable non-uniform image array indexing and a variable descriptor set count
    ...
    if (!initVulkanRenderDeviceWithDescriptorIndex(vk, vkDev, kScreenWidth,
    ...
    for (uint32_t j = 0; j < 3; j++)
    {
        ...
        {
            char fname[1024];
            snprintf(fname, sizeof(fname), "data/explosion%01u/explosion%02u-frame%03u.tga", j, j, i + 1);
