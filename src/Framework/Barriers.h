// To specify memory dependencies without using Vulkan subpasses, we insert pipeline barriers in between
// the rendering commands. Inserting barriers into a command buffer is performed in this file,
// which declared a number of helper classes that essentially just emit the appropriate
// vkCmdPipelineBarrier() function. These helper classes are used in the form of Renderer.
// We declare a variable, initialize that barrier with an appropriate texture handle and flags, and add this
// variable to the list of renderers.
...
{
	ShaderOptimalToColorBarrier(VulkanRenderContext &c, VulkanTexture tex) : Renderer(c),
																			 tex_(tex)
	{
	}
...
{
	ShaderOptimalToDepthBarrier(VulkanRenderContext &c, VulkanTexture tex) : Renderer(c),
																			 tex_(tex)
	{
	}
...
{
	ColorToShaderOptimalBarrier(VulkanRenderContext &c, VulkanTexture tex) : Renderer(c),
																			 tex_(tex)
	{
	}
...
{
	ColorWaitBarrier(VulkanRenderContext &c, VulkanTexture tex) : Renderer(c),
																  tex_(tex)
	{
	}
...
{
	DepthToShaderOptimalBarrier(VulkanRenderContext &c, VulkanTexture tex) : Renderer(c),
																			 tex_(tex)
	{
	}
...
{
	ImageBarrier(VulkanRenderContext &c, VkAccessFlags srcAccess, VkAccessFlags dstAccess, VkImageLayout oldLayout, VkImageLayout newLayout, VkImage image) : Renderer(c),
