...
// The only place where this destructor gets called implicitly is in the
// VulkanRenderContext class
...
// we loaded the textures in an ad hoc fashion, as well as created
// the image and sampler. Here, we will wrap the texture file loading code in a single method
VulkanTexture VulkanResources::loadTexture2D(const char *filename)
{
	...
	// Let's assume that all the loaded images are in the RGBA 8-bit per-channel format.
	// This is enforced by the scene converter tool
	VkFormat format = VK_FORMAT_R8G8B8A8_UNORM;
	// VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL layout since
	// loaded images are intended to be used as inputs for fragment shaders
	transitionImageLayout(vkDev, tex.image.image, format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

	// A major improvement we could
	// make to this routine would be to calculate the MIP levels for the loaded image
...
// By default, a new texture is the size of the output framebuffer. The new texture contains dimensions
// and format information that will be passed to the framebuffer creation routine
VulkanTexture VulkanResources::addColorTexture(int texWidth, int texHeight, VkFormat colorFormat, VkFilter minFilter, VkFilter maxFilter, VkSamplerAddressMode addressMode)
{
	...
	// set a fixed layout for our texture at creation
	// time and we won't change the layout each frame
...
// Rendering to an offscreen depth texture is used for shadow mapping and approximating
// ambient occlusion. The routine is almost the same as addColorTexture(), but
// depthFormat and image usage flags must be different. We must also explicitly specify
// the image layout to avoid performance warnings from validation layers.
VulkanTexture VulkanResources::addDepthTexture(int texWidth, int texHeight, VkImageLayout layout)
{
	...
	// The sampler for the depth textures uses different flags
...
// All the buffers in our renderers are created by calling the addBuffer() routine
...
VulkanBuffer VulkanResources::addBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, bool createMapping)
{
	...
	// The createSharedBuffer() method is called so that we can use the buffer in
	// compute shaders. If successful, the buffer is added to the allBuffers container
...
// use it for direct mesh geometry manipulation, the addVertexBuffer() method has been provided
...
// takes a render pass object and a list of attachments to create a framebuffer
VkFramebuffer VulkanResources::addFramebuffer(RenderPass renderPass, const std::vector<VulkanTexture> &images)
{
	VkFramebuffer framebuffer;

	// extract individual image view objects from a container of VulkanTexture objects
	std::vector<VkImageView> attachments;
	for (const auto &i : images)
		attachments.push_back(i.image.imageView);

	// pass a list of attachments to the creation structure.
	// It's assumed that all the images are the same size, so we will use the size of the first one here
	...
	// store the newly created framebuffer in the allFramebuffers container:
	allFramebuffers.push_back(framebuffer);
	return framebuffer;
}

// for shorter initialization periods for swap chains and full
// screen renderpasses. All the framebuffers associated with a swapchain are added to
// the framebuffer list:
...
// Once it's been created, the render pass is added to our local repository, in the
// allRenderPasses container:
...
// assumes that there is at least one attachment.
// Render passes with empty attachment lists are not supported
RenderPass VulkanResources::addRenderPass(const std::vector<VulkanTexture> &outputs, const RenderPassCreateInfo &ci, bool useDepth)
{
	...
	// A render pass with one color attachment is a special case:
	if (outputs.size() == 1)
	{
...
// A depth-only render pass creation, used for shadow mapping, contains less logic
// and simply redirects to the createDepthOnlyRenderPass() function
