...
{
    // declares usage types, binding points, and accessibility flags for buffers used in all shader stages
    ...
    //  create an immutable descriptor set layout that does not change later:
    ...
    // proceed with the actual descriptorSet creation
    ...
    // for each swapchainImage, we update the corresponding
    // descriptorSet with specific buffer handles. We use one uniform buffer and one
    // shader storage buffer to store combined vertices and indices for the programmable
    // vertex pulling technique
...
// start a RenderPass and emitting a single vkCmdDraw command into the Vulkan command buffer
...
// be called every frame to update the uniform buffer with the model-view matrix
void ModelRenderer::updateUniformBuffer(VulkanRenderDevice &vkDev, uint32_t currentImage,
                                        const void *data, const size_t dataSize)
{
    uploadBufferData(vkDev, uniformBuffersMemory_[currentImage], 0, data, dataSize);
}

ModelRenderer::ModelRenderer(VulkanRenderDevice &vkDev, const char *modelFile,
    ...
    : RendererBase(vkDev, VulkanImage())
{
    ...
    if (!createDepthResources(vkDev, vkDev.framebufferWidth, vkDev.framebufferHeight, depthTexture_) ||
        ...
        !createGraphicsPipeline(vkDev, renderPass_, pipelineLayout_,
                                {"data/shaders/VK02.vert",
                                 "data/shaders/VK02.frag",
                                 "data/shaders/VK02.geom"},
                                &graphicsPipeline_))
    {
...
ModelRenderer::ModelRenderer(VulkanRenderDevice &vkDev, bool useDepth, VkBuffer storageBuffer, VkDeviceMemory storageBufferMemory, uint32_t vertexBufferSize, uint32_t indexBufferSize, VulkanImage texture, VkSampler textureSampler, const std::vector<const char *> &shaderFiles, uint32_t uniformDataSize, bool useGeneralTextureLayout, VulkanImage externalDepth, bool deleteMeshData)
    : useGeneralTextureLayout_(useGeneralTextureLayout), vertexBufferSize_(vertexBufferSize), indexBufferSize_(indexBufferSize), storageBuffer_(storageBuffer), storageBufferMemory_(storageBufferMemory), texture_(texture), textureSampler_(textureSampler), deleteMeshData_(deleteMeshData), RendererBase(vkDev, VulkanImage())
{
    if (useDepth)
    {
