...
{
    //  allocate one geometry data buffer for each swapchain image
...
// checks that our lines list is not empty and then
// calls the uploadBufferData() function to upload the lines' geometry data into
// the GPU buffer
...
// packs two per-frame parameters in a structure and
// calls the usual uploadBufferData() for the current swapchain image
void VulkanCanvas::updateUniformBuffer(VulkanRenderDevice &vkDev, const glm::mat4 &modelViewProj,
                                       float time, uint32_t currentImage)
{
...
// checks the lines' presence and emits vkCmdDraw to
// render all the lines in one draw call
...
// just adds two points to the collection
...
void VulkanCanvas::plane3d(const vec3 &o, const vec3 &v1, const vec3 &v2,
                           int n1, int n2, float s1, float s2, const vec4 &color, const vec4 &outlineColor)
{
