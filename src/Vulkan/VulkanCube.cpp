...
// render 12 triangles representing 6 cube faces
	...
	: RendererBase(vkDev, inDepthTexture)
{
	...
	createImageView(vkDev.device, texture.image, VK_FORMAT_R32G32B32A32_SFLOAT,
					VK_IMAGE_ASPECT_COLOR_BIT, &texture.imageView, VK_IMAGE_VIEW_TYPE_CUBE, 6);
	...
	if (!createColorAndDepthRenderPass(vkDev, true, &renderPass_, RenderPassCreateInfo()) ||
		...
		!createGraphicsPipeline(vkDev, renderPass_, pipelineLayout_,
								{"data/shaders/VKCube.vert", "data/shaders/VKCube.frag"},
								&graphicsPipeline_))
	{
