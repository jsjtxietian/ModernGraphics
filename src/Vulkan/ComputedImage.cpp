...
ComputedImage::ComputedImage(VulkanRenderDevice &vkDev, const char *shaderName, uint32_t textureWidth, uint32_t textureHeight, bool supportDownload)
	: ComputedItem(vkDev, sizeof(uint32_t)), computedWidth(textureWidth), computedHeight(textureHeight), canDownloadImage(supportDownload)
{
...
bool ComputedImage::createDescriptorSet()
{
	...
	std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
		VkWriteDescriptorSet{.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .pNext = nullptr, .dstSet = descriptorSet, .dstBinding = 0, .dstArrayElement = 0, .descriptorCount = 1, .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, .pImageInfo = &imageInfo, .pBufferInfo = nullptr, .pTexelBufferView = nullptr},
