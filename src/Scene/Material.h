...
struct PACKED_STRUCT MaterialDescription final
{
    ...
    // Two components, .x and .y, can be used to represent anisotropic roughness when necessary
    ...
    // maps
    // In Vulkan, the texture is addressed by a 32-bit integer in a big texture array, while
    // in OpenGL, the texture is an opaque 64-bit handle provided by the OpenGL
    // implementation via the ARB_bindless_texture extension
    ...
    // The opacity map is only used during conversion and has only been included because
    // we are using the same structure in our SceneConverter tool
